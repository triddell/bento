...
// TestOptionalDateTimestampDoesNotPanic reproduces a real crash found via live testing: an
// optional DATE or TIMESTAMP_MILLIS field, with optionalAsPtrs enabled (as Harbor's real
// aws_s3_parquet_stream output config does), produced a *int32/*int64 struct field. parquet-go's
// "date"/"timestamp" struct tags only accept a pointer when the pointee is time.Time, so this
// panicked via throwInvalidTag for every real Harbor field marked optional (processed_time,
// start_time, end_time) even though the equivalent non-optional fields worked fine. wrapType must
// keep the base type unwrapped (plain int32/int64) for these two tokens regardless of
// optionalAsPtrs.
